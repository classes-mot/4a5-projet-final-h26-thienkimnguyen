...
function App() {
  ...
  const addAvisHandler = (avisData) => {
    const newEntry = {
      id: Math.random().toString(),
      description: avisData.description,
      creator: avisData.creator,
    };

    setReviews((prevReviews) => [newEntry, ...prevReviews]);
  };

  const deleteAvisHandler = (idToDelete) => {
    setReviews((prevReviews) =>
      prevReviews.filter((avis) => avis.id !== idToDelete),
    );
  };
  ...
  const routerLoggedIn = createBrowserRouter([
    {
      ...
      children: [
        ...
        { path: "/review/edit/:reviewId", element: <NewReview></NewReview> },
      ],
    },
  ]);
